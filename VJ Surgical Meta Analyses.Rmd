...
forest(df.PR.rma, 
       ...
       rows=c(3:(3+22), (30+gap):(30+14+gap), (49+gap):(49+13+gap), (49+13+5+gap):(74+gap), (79+gap):(92+gap), (97+gap):(97+8-1+gap), (97+8-1+5+gap):(110+gap), (115+gap):(115+gap)),
       cex=0.3,
...
### add text with Q-value, dfs, p-value, and I^2 statistic
text(x_begin, -2.5, pos=4, cex=0.75, bquote(paste("RE for All Studies (Q = ",
     .(formatC(df.PR.rma$QE, digits=2, format="f")), ", df = ", .(df.PR.rma$k - df.PR.rma$p),
     ", p = ", .(formatC(df.PR.rma$QEp, digits=2, format="f")), "; ", I^2, " = ",
     .(formatC(df.PR.rma$I2, digits=1, format="f")), "%)")))
...
### add text for the test of subgroup differences
text(x_begin, -5.5, pos=4, cex=0.75, bquote(paste("Test for Subgroup Differences: ",
     Q[M], " = ", .(formatC(res$QM, digits=2, format="f")), ", df = ", .(res$p - 1),
     ", p = ", .(formatC(res$QMp, digits=2, format="f")))))
...
### add text for the subgroups
text(x_begin, cex=0.5, c(26, 45, 63, 75, 93, 105, 111, 116), pos=4, c("ATL",
                                                             "Gamma Knife",
                                                             "LiTT",
                                                             "Radiofrequency",
                                                             "SAH Transcortical",
...
addpoly(res.SAH, row=106.5, cex=0.3, mlab="")
addpoly(res.ts, row= 94.5, cex=0.3, mlab="")
addpoly(res.tc, row= 76.5, cex=0.3, mlab="")
addpoly(res.rf, row= (49+13+2.5), cex=0.3, mlab="")
addpoly(res.litt, row= 46.5, cex=0.3, mlab="")
addpoly(res.gn, row=27.5, cex=0.3, mlab="")
...
text(x_begin, 106.5, pos=4, cex=0.3, bquote(paste("RE Model for Subgroup (Q = ",
...
res.s <- res.ts
text(x_begin, 94.5, pos=4, cex=0.3, bquote(paste("RE Model for Subgroup (Q = ",
...
res.s <- res.tc
text(x_begin, 76.5, pos=4, cex=0.3, bquote(paste("RE Model for Subgroup (Q = ",
     .(formatC(res.s$QE, digits=2, format="f")), ", df = ", .(res.s$k - res.s$p),
     ", p = ", .(formatC(res.s$QEp, digits=2, format="f")), "; ", I^2, " = ",
     .(formatC(res.s$I2, digits=1, format="f")), "%)")))

res.s <- res.rf
text(x_begin, (49+13+2.5), pos=4, cex=0.3, bquote(paste("RE Model for Subgroup (Q = ",
     .(formatC(res.s$QE, digits=2, format="f")), ", df = ", .(res.s$k - res.s$p),
     ", p = ", .(formatC(res.s$QEp, digits=2, format="f")), "; ", I^2, " = ",
     .(formatC(res.s$I2, digits=1, format="f")), "%)")))

res.s <- res.litt
text(x_begin, 46.5, pos=4, cex=0.3, bquote(paste("RE Model for Subgroup (Q = ",
     .(formatC(res.s$QE, digits=2, format="f")), ", df = ", .(res.s$k - res.s$p),
     ", p = ", .(formatC(res.s$QEp, digits=2, format="f")), "; ", I^2, " = ",
     .(formatC(res.s$I2, digits=1, format="f")), "%)")))

res.s <- res.gn
text(x_begin, 27.5, pos=4, cex=0.3, bquote(paste("RE Model for Subgroup (Q = ",
     .(formatC(res.s$QE, digits=2, format="f")), ", df = ", .(res.s$k - res.s$p),
     ", p = ", .(formatC(res.s$QEp, digits=2, format="f")), "; ", I^2, " = ",
     .(formatC(res.s$I2, digits=1, format="f")), "%)")))

res.s <- res.atl
text(x_begin, 1, pos=4, cex=0.3, bquote(paste("RE Model for Subgroup (Q = ",
     .(formatC(res.s$QE, digits=2, format="f")), ", df = ", .(res.s$k - res.s$p),
     ", p = ", .(formatC(res.s$QEp, digits=2, format="f")), "; ", I^2, " = ",
     .(formatC(res.s$I2, digits=1, format="f")), "%)")))
