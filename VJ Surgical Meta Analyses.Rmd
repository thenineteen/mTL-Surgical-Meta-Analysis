...
forest(df.PR.rma, 
       ...
       # showweights = TRUE,
       ylim=c(-1, 120),
       rows=c(3:(3+22), 30:(30+14), 49:(49+13), (49+13+5):74, 79:92, 97:(97+8-1), (97+8-1+5):(110), 115:115),
       cex=0.3,
       addcred=TRUE, level=95,
       )


### add text with Q-value, dfs, p-value, and I^2 statistic
overall_text <- text(-16, -1, pos=4, cex=0.75, bquote(paste("RE for All Studies (Q = ",
     .(formatC(df.PR.rma$QE, digits=2, format="f")), ", df = ", .(df.PR.rma$k - df.PR.rma$p),
     ", p = ", .(formatC(df.PR.rma$QEp, digits=2, format="f")), "; ", I^2, " = ",
     .(formatC(df.PR.rma$I2, digits=1, format="f")), "%)")))
...
### add text for the test of subgroup differences
text(-16, -2.8, pos=4, cex=0.75, bquote(paste("Test for Subgroup Differences: ",
     Q[M], " = ", .(formatC(res$QM, digits=2, format="f")), ", df = ", .(res$p - 1),
     ", p = ", .(formatC(res$QMp, digits=2, format="f")))))
...
### add text for the subgroups
text(-16, c(24, 16, 5), pos=4, c("Radiofrequency",
                               "ATL",
                               "Gamma Knife"))
...
# and save
dev.copy(device=svg, "Figures\\subplots_df_PR.svg", width=10, height=8)
dev.off()
```
